...
Attempts to fit multiple gaussians, then plot
...
Obs_initial = [5000.0, 0.5, 4.0, 5000.0, 0.5, 4.0]
Den_initial = [7000.0, 0.5, 4.0, 7000.0, 0.5, 4.0]
...
Obs_optimal, obscov = scopt.curve_fit(fep.double_gaussian, Obs_dist[:,0], Obs_dist[:,1], Obs_initial)
Den_optimal, dencov = scopt.curve_fit(fep.double_gaussian, Den_dist[:,0], Den_dist[:,1], Den_initial)

##Calculated a Chi-Squared Test on the fitted gaussians
for n in Obs_dist[:,0]:
    yobs = np.append(yobs, fep.double_gaussian(n, Obs_optimal[0], Obs_optimal[1], Obs_optimal[2], Obs_optimal[3], Obs_optimal[4], Obs_optimal[5]))
for n in Den_dist[:,0]:
    yden = np.append(yden, fep.double_gaussian(n, Den_optimal[0], Den_optimal[1], Den_optimal[2], Den_optimal[3], Den_optimal[4], Den_optimal[5]))

print "The chisquare from scipy is = ", stat.chisquare(Obs_dist[:,0], yobs)
...
print "Ideal observed parameters = ", Obs_optimal
print "Ideal denoised parameters = ", Den_optimal
...
plt.savefig("Fitted_double-Observed-R-distribution.pdf")
...
plt.savefig("Fitted_double-Denoised-R-distribution.pdf")
